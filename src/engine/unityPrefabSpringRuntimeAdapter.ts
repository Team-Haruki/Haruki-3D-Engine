...
export class UnityPrefabSpringRuntime {
  ...
  update(deltaTime: number): void {
    ...
    const lateUpdateProviders = this.collectWindLateUpdateProviders();
    ...
    for (const provider of lateUpdateProviders) {
  ...
  private collectWindLateUpdateProviders(): RuntimeWindVolumeOneSelf[] {
    ...
    for (const bone of this.bones) {
      for (const provider of bone.forceProviders) {
        if (provider.kind !== "WindVolumeOneSelf" || provider.springManagerPathId === null) {
          continue;
        }
  ...
  private computeWindVolumeOneSelfForce(
    ...
    const waveScale = Math.PI * 2 / provider.peakDistance;
    const wave = Math.sin(
      phase +
